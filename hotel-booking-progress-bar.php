...
function mphbpb_menu_option() {
	add_submenu_page(
		'options-general.php',
		'Hotel Booking Progress Bar',
		'Booking Progress Bar',
		'manage_options',
		'hotel_booking_progress_bar_menu',
		'mphbpb_menu_page',
		200
	);
}
 ...
 */
function mphbpb_enqueue_color_picker() {
...
function mphbpb_DB_query() {
	$dbValues = array(
		'step1' => wp_kses_post( stripslashes( get_option( 'mphbpb_step_1' , '<strong>' . __( 'Step 1:', 'mphbpb' ) . '</strong><br />' . __( 'Search Results', 'mphbpb' ) ) ) ),
		'step2' => wp_kses_post( stripslashes( get_option( 'mphbpb_step_2' , '<strong>' . __( 'Step 2:', 'mphbpb' ) . '</strong><br />' . __( 'Booking Details', 'mphbpb') ) ) ),
		'step3' => wp_kses_post( stripslashes( get_option( 'mphbpb_step_3' , '<strong>' . __( 'Step 3:', 'mphbpb' ) . '</strong><br />' . __( 'Booking Confirmed', 'mphbpb' ) ) ) ),
...
function mphbpb_menu_page() {
	...
	if( array_key_exists( 'submit_mphbpb_changes' , $_POST ) ){
		...
		if ( !wp_verify_nonce( $retrieved_nonce , 'update_mphbpb_settings' ) ){ 
			...
			<div id="setting-error-settings_updated" class="notice notice-error is-dismissible">
				<p>
					<strong><?php _e('There was an error and the data was not saved! Please try again.', 'mphbpb') ?></strong>
				</p>
			</div>
			<?php
		} else {
			...
			<div id="setting-error-settings_updated" class="notice notice-success is-dismissible">
				<p>
					<strong><?php _e( 'Settings have been saved', 'mphbpb' ); ?></strong>
				</p>
			</div>
	...
	?>
		<div class="wrap">
			<h2 style="margin-top: 50px;"><?php _e( 'Hotel Booking Progress Bar Settings', 'mphbpb' ); ?></h2>
			<?php if ( is_plugin_active( 'motopress-hotel-booking/motopress-hotel-booking.php' ) ) { ?>
				<br>
				<h3><?php _e( 'Progress bar colors', 'mphbpb' ); ?></h3>
				<br>
				<form action="" method="post">
					...
					?> />
					<label for="mphbpb_show_submitted"><?php _e( 'Show step 3 as complete on the "reservation submitted" page', 'mphbpb' ); ?></label>
					<p><?php _e( 'This option is especially for bookings that require admin or guest confirmation to complete, and therefore show a "reservation submitted" page instead of the "booking confirmed" page. If bookings are confirmed directly, this option will have no affect.', 'mphbpb' ); ?>
					<br><br><br><br>
					<input type="submit" name="submit_mphbpb_changes" value="<?php _e( 'Save Settings', 'mphbpb' ); ?>" class="button button_primary primary">
				</form>
				
			<?php }else{ ?>
				<h3><?php _e( 'Please note that the MotoPress Hotel Bookings plugin is required for this plugin', 'mphbpb' ); ?></h3>
			<?php } ?>
		</div>
...
function mphbpb_add_scripts() {
	...
	if( is_plugin_active( 'motopress-hotel-booking/motopress-hotel-booking.php' )){
		...
		if( is_page( $searchResultsPage ) || is_page( $bookingDetailsPage ) || is_page( $bookingConfirmedPage ) ){
			...
			wp_enqueue_script( 'mphbpb_insert_progress_bar', plugins_url('/assets/js/addProgressBar.js', __FILE__ ), array( 'jquery' ), '', true );
...
function mphbpb_add_settings_link( $links ) {
	...
	$settings_link = "<a href=$url>" . __( 'Settings', 'mphbpb' ) . "</a>";
